...
| Rule set | the 36 bundled `.guard` files under `rules/` |
...
| --- | --- | --- | --- | --- |
| a | Every applicable rule passes | `0` | 36 JSON records, one per rule file, `status` `PASS` or `SKIP`, all `not_compliant` empty | empty |
...
Case a above yields `rules_evaluated = 36`, matching the 36 bundled `.guard`
